...
public class MissionMapper implements Function<Mission, MissionDTO> {
    ...
    public Mission toEntity(MissionDTO dto) {
        ...
        Optional.ofNullable(dto.getStatus())
                .ifPresent(status -> mission.setStatus(statusMapper.toEntity(status)));
        ...
        mission.setTransports(
                dto.getTransportIds().stream()
                        .map(id -> transportService.getTransportById(id)).collect(Collectors.toSet())
        );
